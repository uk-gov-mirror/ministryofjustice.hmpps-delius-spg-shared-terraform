...
locals {
  ...
  roles_allowed_to_decrypt_spg_tls = [
    data.terraform_remote_state.iam.outputs.iam_policy_iso_ext_app_role_arn,
    data.terraform_remote_state.iam.outputs.iam_policy_mpx_int_app_role_arn,
  ]

  roles_allowed_to_decrypt_spg_signing = [
    data.terraform_remote_state.iam.outputs.iam_policy_iso_ext_app_role_arn, //whilst spg-iso smx in place, remove when using haproxy + mpxhybrid
    data.terraform_remote_state.iam.outputs.iam_policy_mpx_int_app_role_arn,
  ]

  roles_allowed_to_decrypt_crc = [
    data.terraform_remote_state.iam.outputs.iam_policy_iso_ext_app_role_arn, //for when using all in one
    data.terraform_remote_state.iam.outputs.iam_policy_mpx_int_app_role_arn, //for when using all in one
    data.terraform_remote_state.iam.outputs.iam_policy_crc_int_app_role_arn,
  ]
