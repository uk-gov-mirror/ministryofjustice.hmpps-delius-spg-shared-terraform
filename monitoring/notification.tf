...
resource "aws_cloudwatch_metric_alarm" "environment_severe" {
  alarm_name                = "${local.environment_name} environment health severe"
  ...
  statistic                 = "Maximum"
  threshold                 = "25"
  alarm_description         = "The ${local.environment_name} environment is Warning or higher"
  alarm_actions             = ["${aws_sns_topic.alarm_notification.arn}"]
}

resource "aws_cloudwatch_metric_alarm" "environment_warning" {
  alarm_name                = "${local.environment_name} environment health warning"
  comparison_operator       = "GreaterThanOrEqualToThreshold"
...
resource "aws_cloudwatch_metric_alarm" "latency" {
  alarm_name                = "${local.environment_name} environment performance"
  ...
  statistic                 = "Average"
  threshold                 = "1"
  alarm_description         = "The response latency for the ${local.environment_name} environment is over 1s"
  alarm_actions             = ["${aws_sns_topic.alarm_notification.arn}"]
}
...
resource "aws_sns_topic" "alarm_notification" {
  name               = "${local.environment_name}_alarm_notification"
}
