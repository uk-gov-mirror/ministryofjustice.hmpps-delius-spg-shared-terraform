...
module "spg_tls" {
  ...
  alfresco_app_name                        = "spgw-ext.${local.spg_app_name}"
  ...
  common_name                              = "spgw-ext.${local.common_name}"
  environment_identifier                   = "${local.environment_identifier}"
  internal_domain                          = "spgw-ext.${local.internal_domain}"
  region                                   = "${local.region}"
...
####################################################
# key
output "spg_tls_private_key" {
  value     = "${module.spg_tls.self_signed_server_private_key}"
  sensitive = true
}

# csr
output "spg_tls_cert_request_pem" {
  value     = "${module.spg_tls.self_signed_server_cert_request_pem}"
  sensitive = true
}

# cert
output "spg_tls_cert_pem" {
  value = "${module.spg_tls.self_signed_server_cert_pem}"
}
