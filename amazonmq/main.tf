locals {
  ...
  int_amq_security_groups = data.terraform_remote_state.vpc-security-groups.outputs.sg_amazonmq_in

  private_subnet_ids = [
    data.terraform_remote_state.common.outputs.private_subnet_ids[0],
    data.terraform_remote_state.common.outputs.private_subnet_ids[1]
  ]
  ...
  credentials_ssm_path = "/${data.terraform_remote_state.vpc.outputs.environment_name}/${data.terraform_remote_state.vpc.outputs.tags.application}/weblogic/spg-domain"
...
resource "aws_route53_record" "dns_spg_amq_a_int_entry" {
  ...
  zone_id = data.terraform_remote_state.common.outputs.private_zone_id
  name    = "amazonmq-broker-1"
  type    = "CNAME"
  ttl     = "1800"
  count   = 1
  records = [substr(
    element(split(":", aws_mq_broker.SPG.instances[0].endpoints[0]), 1),
    2,
    -1,
  )]
...
resource "aws_route53_record" "dns_spg_amq_b_int_entry" {
  ...
  # returns a single list item then leave it as-is and remove this TODO comment.
  records = [replace(
   sort(aws_route53_record.dns_spg_amq_a_int_entry.0.records)[0],
    "-1.mq",
    "-2.mq",
  )]
  depends_on = [aws_mq_broker.SPG]
...
data "null_data_source" "broker_export_friendly_url" {
  inputs = {
    broker_connect_url = local.broker_instances == 1 ? format(
      "ssl://%s:%s",
      aws_route53_record.dns_spg_amq_a_int_entry[0].fqdn,
      data.null_data_source.broker_export_port.outputs["broker_ssl_port"],
      ) : format(
      "failover:(ssl://%s:%s,ssl://%s:%s)",
      aws_route53_record.dns_spg_amq_a_int_entry[0].fqdn,
      data.null_data_source.broker_export_port.outputs["broker_ssl_port"],
      element(
        concat(aws_route53_record.dns_spg_amq_b_int_entry.*.fqdn, [""]),
        0,
      ),
      data.null_data_source.broker_export_port.outputs["broker_ssl_port"],
    )
...
data "null_data_source" "broker_export_full_url" {
  inputs = {
    broker_connect_url = local.broker_instances == 1 ? format("failover:(%s)", aws_mq_broker.SPG.instances[0].endpoints[0]) : format(
      "failover:(%s,%s)",
