...
data "template_file" "iam_policy_app_mpx_int" {
  ...
  vars = {
    ...
    app_role_arn                 = data.terraform_remote_state.iam.outputs.iam_policy_mpx_int_app_role_arn
...
module "create-iam-app-policy-mpx-int" {
  ...
  rolename   = data.terraform_remote_state.iam.outputs.iam_policy_mpx_int_app_role_name
}
