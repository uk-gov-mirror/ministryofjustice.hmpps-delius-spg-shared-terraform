...
data "template_file" "iam_policy_app_crc_int" {
  ...
  vars = {
    ...
    app_role_arn                 = data.terraform_remote_state.iam.outputs.iam_role_crc_int_ecs_role_arn
...
module "create-iam-app-policy-crc-int" {
  ...
  rolename   = data.terraform_remote_state.iam.outputs.iam_policy_crc_int_app_role_name
}
